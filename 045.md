...
```
In the below function, the vault is upgraded to Version 2, and during this update, the uniswapV3Ref will not be set as it was initialised in the constructor during version 1, instead of initialize().
...
b) Avoid using immutable for the variable in up gradable contracts. If Immutable is removed, the reference will not be held in even version 1. It looked like a workaround for version1 to work.
